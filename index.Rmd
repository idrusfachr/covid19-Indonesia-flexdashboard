...
case_by_prov <- ina_province %>%
  ...
  mutate(Tanggal = as.Date(Tanggal)) %>%
  filter(Tanggal != "2020-08-03" & Tanggal != "2020-08-10" & Tanggal != "2020-08-11" & Tanggal != "2020-08-12" & Tanggal != "2020-08-13" & Tanggal != "2020-08-14") %>%
  arrange(Tanggal) %>% 
  group_by(Provinsi) %>%
...
> NOTE : Terdapat anomali di tanggal 3. 9 dan 11 Agustus pada data penambahan kasus perprovinsi yang bersumber dari BNPB. Perbedaan data yang ada hingga ratusan kali lipat dengan data penambahan secara nasional di tanggal yang sama, juga berbeda dengan data penambahan harian resmi di tiap provinsi. Karena average sensitif terhadap outlier, maka data tanggal di-exclude dari perhitungan rolling average.
