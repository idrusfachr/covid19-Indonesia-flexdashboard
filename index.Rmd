...
Visualisasi perkembangan kasus Covid-19 di Indonesia. Data bersumber dari 
...
ina_daily <- getDailyData(INA_DAILY) %>% cleanData()
ina_province <- getProvinceData(INA_PROVINCE) %>% cleanData()

update_date <- max(ina_province$Tanggal)
```
...
```{r}
confirmed <- ina_daily %>% subset(Tanggal==update_date, select = Jumlah_Kasus_Kumulatif) %>%
    format(round(0), big.mark = ",")

confirmed_new <- ina_daily %>% subset(Tanggal==update_date, select = Jumlah_Kasus_Baru_per_Hari) %>%
    format(round(0), big.mark = ",")
...
```{r}
hospitilized <- ina_daily %>% subset(Tanggal==update_date, select = Jumlah_pasien_dalam_perawatan) %>%
    format(round(0), big.mark = ",")

hospitilized_new <- ina_daily %>% subset(Tanggal==update_date, select = Jumlah_Kasus_Dirawat_per_Hari) %>%
    format(round(0), big.mark = ",")
...
```{r}
recovered <- ina_daily %>% subset(Tanggal==update_date, select = Jumlah_Pasien_Sembuh) %>%
    format(round(0), big.mark = ",")

recovered_new <- ina_daily %>% subset(Tanggal==update_date, select = Jumlah_Kasus_Sembuh_per_Hari) %>%
    format(round(0), big.mark = ",")
...
```{r}
death <- ina_daily %>% subset(Tanggal==update_date, select = Jumlah_Pasien_Meninggal) %>%
    format(round(0), big.mark = ",")

death_new <- ina_daily %>% subset(Tanggal==update_date, select = Jumlah_Kasus_Meninggal_per_Hari) %>%
    format(round(0), big.mark = ",")
...
dygraph_df <- ina_daily %>% 
  select(Tanggal, Jumlah_Kasus_Kumulatif, Jumlah_Pasien_Sembuh, Jumlah_Pasien_Meninggal)%>%
  filter(Tanggal < update_date + 1) %>%
  mutate(Tanggal=as.Date(Tanggal)) %>%
  rename(TERKONFIRMASI = Jumlah_Kasus_Kumulatif, SEMBUH = Jumlah_Pasien_Sembuh, MENINGGAL = Jumlah_Pasien_Meninggal)
...
dygraph_df <- ina_daily %>% 
  filter(Tanggal < update_date + 1) %>%
  select(Tanggal, Jumlah_Kasus_Baru_per_Hari, Jumlah_Kasus_Sembuh_per_Hari, Jumlah_Kasus_Meninggal_per_Hari)%>%
  mutate(Tanggal=as.Date(Tanggal)) %>%
  rename(TERKONFIRMASI = Jumlah_Kasus_Baru_per_Hari, SEMBUH = Jumlah_Kasus_Sembuh_per_Hari, MENINGGAL = Jumlah_Kasus_Meninggal_per_Hari)
...
prov_status <- ina_province %>% 
  select(Provinsi, Tanggal, Kasus_Terkonfirmasi_Akumulatif, Kasus_Sembuh_Akumulatif, Kasus_Meninggal_Akumulatif, Kasus_Aktif_Akumulatif)  %>% 
    filter(Provinsi != "Indonesia") %>%
    filter(as.Date(Tanggal) == update_date) %>%
...
prov_status$Status <- with(prov_status, ifelse(
  death_rate < mean(prov_status$death_rate) & recovery_rate < mean(prov_status$recovery_rate), 'LowDeathHighRecover', ifelse(
 ...
 death_rate > mean(prov_status$death_rate) & recovery_rate > mean(prov_status$recovery_rate), 'HighDeathHighRecover', ifelse(
  death_rate < mean(prov_status$death_rate) & recovery_rate > mean(prov_status$recovery_rate), 'LowDeathHighRevover', 
  'Others')))))

colors <- c('#FFD300', '#C53E13', '#FFC800', '#5EFF00', '#FFC100')
...
hchart(
  ...
  hc_caption(
    text = sprintf("Last Update : %s", update_date)
    ) %>%
  hc_credits(
    enabled = TRUE, text = "Graph by idrusfachr build with highcharter",
    href = "idrusfachr.netlify.com",
